...
import {commentBttnImg, likeBttnImg, likedBttnImg, menuImg, shareBttnImg} from '_assets';
...
const FeedPost = ({allComments, post, navigation}) => {
  ...
  const LIKE_POST = gql`
    mutation LikePost($userId: String!, $postId: String!, $postCreator: String!, $isUnlike: Boolean!) {
      likePost(userId: $userId, postId: $postId, postCreator: $postCreator, isUnlike: $isUnlike)
    }
  `;
  ...
  const handleSubmit = () => {
    ...
    if(!isActive){
      ...
      post.likes == null ? post.likes = null : post.likes = post.likes.filter(function(p) { 
        return p != userId; 
      })
    }
  ...
  useEffect(() => {
    if (isSubmitting) {
      likePost({variables: {userId: userId, postCreator: post.createdBy, isUnlike: !isActive, postId: post.postId},
        // update: (cache) => {
        //   try {
    ...
    <View style={styles.feedPostContainer}>
      ...
      <TouchableOpacity style={styles.feedPostHeaderContainer}>
        ...
        <View style={styles.feedPostHeaderTextContainer}>
          ...
          <Text style={styles.h3d1}>{post.createdBy + ' ' + postTypeText(post.postType, classData[0])}</Text>
          <Text style={styles.p2d2}>{`${moment(parseInt(post.createdAt)).fromNow()}`}</Text>
        </View>
        {/* <TouchableOpacity>
      ...
      <View style={styles.postDetailsContainer}>
        ...
        <View style={styles.classDetailsContainer}>
          <MedHortClassCard navigation={navigation} classData={classData} />
        </View>
      </View>
      ...
      <View style={styles.likeContainer}>
        {/* # likes & comments */}
        <Text style={styles.p1d1}>{post.likes == null ? 0 + " likes" : post.likes.length + ' likes'}</Text>
      ...
      <View style={styles.interactionButtonContainer}>
        <View style={styles.likeCommentContainer}>
          <TouchableOpacity style={styles.likeButtonContainer} onPress={handleSubmit}>
            <Image source={isActive ? likedBttnImg : likeBttnImg} style={styles.iconLarge} />
          </TouchableOpacity>
          <TouchableOpacity>
