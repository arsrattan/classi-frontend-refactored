...
const FollowButton = ({followedUser, isUnfollow, active}) => {
  ...
  const FOLLOW_USER = gql`
    mutation ToggleFollow($userId: String!, $followedUser: String!, $isUnfollow: Boolean!) {
      toggleFollow(userId: $userId, followedUser: $followedUser, isUnfollow: $isUnfollow)
    }
  `;
  ...
  useEffect(() => {
    if (isSubmitting) {
      followUser({variables: {userId: userId, followedUser: followedUser, isUnfollow: isUnfollow},
        update: (cache) => {
          try {
            let following = cache.readQuery({ query: GET_USER_FOLLOWING, variables: {userId: userId}});
  ...
  return (
    <TouchableOpacity style={isActive ? styles.followingBtn : styles.followBtn } onPress={handleSubmit}>
      <Text style={isActive ? styles.followingTxt : styles.followTxt }>{isActive ? 'Following' : 'Follow Back'}</Text>
    </TouchableOpacity>
  )}
;
